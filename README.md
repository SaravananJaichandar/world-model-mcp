...
### v0.11 (Planned) — Depth after breadth

Depth release. v0.10 expanded surface area to seven runtimes; v0.11 solves real problems for the users we now have rather than adding more surfaces. Two signals shaped this list: [Hermes #47349 (2026-07-01)](https://github.com/NousResearch/hermes-agent/issues/47349) surfaced the write-side routing gap (MCP surfaces tools but the agent still chooses the destination); and the `auto` strategy on the v0.8.1 contradiction-resolution benchmark still scores 77.1% because it does not fully consume the `confirmer` + decay-awareness fields shipped in v0.8.0.

- [ ] **v0.11.0 A: `auto` strategy rewrite for `resolve_contradiction`.** Fold in `confirmer` + per-evidence-type decay awareness. Goal: lift the v0.8.1 contradiction-resolution benchmark's `auto` score from 77.1% past 90%. Uses the existing 105-pair × 19-category benchmark as the test bed — measurable outcome, self-contained code change, immediate signal loop.
- [ ] **v0.11.0 B: Hermes native `MemoryProvider` plugin.** Python plugin implementing Hermes' `agent/memory_provider.py` ABC (`initialize`, `get_tool_schemas`, `handle_tool_call`, `get_config_schema`, `save_config`). Intercepts writes at Hermes' routing layer rather than only surfacing tools. Priority bumped from "conditional on MCP adoption" to planned after [#47349](https://github.com/NousResearch/hermes-agent/issues/47349) demonstrated real user demand for write-side interception (not just tool availability).
- [ ] **v0.11.1: Content-type routing distinction (rules / facts / procedures).** Add an explicit content-type axis to the schema so writes can be routed intelligently by a MemoryProvider (rules → always-inject store, facts → search-on-demand store, procedures → skills store). Currently the schema has `evidence_type` (test/bug_fix/user_correction/source_code/session) but no rules-vs-facts-vs-procedures axis. Sourced from Hermes #47349 architectural framing.
- [ ] **v0.11.2: Dogfooding case study.** Publish the constraints, facts, and decision traces world-model-mcp captured about itself while building v0.10 and v0.11 through Claude Code. Real evidence for the "the project dogfoods its own memory layer" claim, not just an assertion.
