...
> **Latest: v0.15.5** — Streaming offline verifier + FIPS 205 SLH-DSA known-answer tests + adversarial-input fuzz on the verify path. `verify_manifest_streaming(file_path)` in `world_model_server.etch_verify` parses the manifest row-by-row via `ijson`, capping verify memory at O(single row + compact row_hash lookup) instead of O(manifest size). Closes the last remaining OOM path in the verify pipeline — the pre-v0.15.5 hosted attestation timer streamed the export but still `json.load`'d the file back and pinned multi-GB RSS on a 760MB manifest. `etch-verify` CLI now uses streaming by default. FIPS 205 KAT tests lock parameter sizes for SLH-DSA-SHA2-128f (pk = 32 bytes, sk = 64 bytes, sig = 17,088 bytes), the sign/verify functional round-trip, and a fixed KAT-vector fixture that must verify true forever. Atheris fuzz targets on `verify_manifest` + `verify_manifest_streaming` plus a 258-case non-Atheris smoke fuzz that runs in every CI pass. Coverage floor bumped 69/64 → 71/66. Ruff hard gate + mypy strict deferred to a dedicated cleanup PR. Ships on top of v0.15.4 (streaming exporter), v0.15.3 (verify_slh_dsa semantic fix), v0.15.2 (concurrent-append race fix), v0.15.1 (`etch-verify` CLI + `audit_dump`), and v0.15.0 (`pin_annotation`). See [full version history](#full-version-history) below.
...
- **`verify_manifest_streaming(file_path)` in `world_model_server.etch_verify`.** Parses the manifest row-by-row via `ijson` so verify memory usage is O(single row + compact row_hash lookup) rather than O(manifest size). Same verdict, same check names, same counts as the in-memory `verify_manifest(dict)` for the same bytes.
- **`etch-verify` CLI switches to streaming by default.** `--in-memory` remains as a debugging escape hatch. `manifest_sha256` is computed in a 64KB-chunked streaming pass so hashing a 10GB manifest does not require the whole file in RAM.
- **Hosted-side attestation timer is fully streaming.** Pre-v0.15.5 already streamed the export but then `json.load`'d the file back into a Python dict for verify — verified 2026-07-24 to pin multi-GB RSS on a 760MB manifest, forcing preflight_skipped rows. v0.15.5 removes the `json.load`, so memory stays bounded through export + hash + verify.
- **FIPS 205 SLH-DSA known-answer tests in `tests/test_fips_205_slh_dsa_kat.py`.** Locks parameter sizes for SLH-DSA-SHA2-128f (public key = 32 bytes, secret key = 64 bytes, signature = 17,088 bytes), sign/verify round-trip with wrong-key + wrong-message + mutated-signature rejection, and a fixed KAT-vector fixture (`tests/fixtures/slh_dsa_kat_vectors.json`) that must verify true forever. Byte-format equivalence across the FIPS 205 and legacy SPHINCS+ mechanism names is also locked.
- **Atheris fuzz targets in `fuzz/fuzz_verify_manifest.py`.** Feeds random and mutated bytes into both verify entry points, asserts no unhandled exception, no hang, no memory blow-up. Seed corpus checked into `fuzz/corpus/`. Run locally with `python fuzz/fuzz_verify_manifest.py fuzz/corpus/ -max_total_time=60`.
- **Non-Atheris smoke fuzz in `tests/test_fuzz_smoke_verify.py`.** Same invariant exercised via 29 adversarial payloads plus 100 deterministic-random 200-byte seeds against both verify paths. Runs in every CI pass without needing Clang / libFuzzer.
- **Pre-existing env-leak flake fixed in `tests/test_operational_e2e.py`.** Hard-kill recovery test was mutating `os.environ` directly in the parent process without cleanup, leaking `WORLD_MODEL_AUDIT_LOG` into subsequent tests under random ordering. Fixed to use `mock.patch.dict` for scoped env cleanup.
- **Coverage floor bumped 69/64 → 71/66** in `pytest.yml` to match the 2026-07-24 baseline of 72.06% across `world_model_server`. Two-tier floor stays in place (lower tier for CI runners without SLH-DSA in liboqs).
- **Deferred:** ruff hard gate + mypy strict. ~745 pre-existing typing-deprecation warnings across the source tree need a dedicated cleanup PR before either can be turned on as a hard gate. Landing separately.
...
- **First-party adapter docs for [Mesh-LLM](docs/adapters/mesh-llm.md), [BUZZ](docs/adapters/buzz.md), and [Goose](docs/adapters/goose.md).** BUZZ is verified end-to-end by a real integration test (`tests/integration/test_buzz_acp_handshake.py`) that spawns a real `buzz-agent` binary + real world-model-mcp Python subprocess + a local fake OpenAI-compatible LLM, then asserts a `query_fact` tool call round-trips cleanly. A second integration test verifies multi-agent handoff at N=5: four buzz-agent processes each write a `record_decision` to a shared world-model-mcp DB, a fifth agent (fresh process, distinct ACP session id) reads all four back via `get_decision_log`. Runs in ~5 seconds; skips cleanly when `BUZZ_AGENT_BIN` is unset (the one class of skip explicitly documented as legitimate).
- **Adapter docs validation contract.** Every doc under `docs/adapters/` is now enforced by `tests/test_adapters_docs_config.py` (18 tests). Any `WORLD_MODEL_*` env var referenced in a doc must be one that `Config` actually reads; every fenced JSON/YAML block must parse; the `world-model` MCP server invocation must match the reference config shipped in `world_model_server/adapters/cursor/mcp.json`. Rule: no new adapter doc merges without matching validation tests. Two BUZZ-specific schema gotchas caught by writing the integration test are pinned by regression tests: `mcpServers[].env` is a list of `{name, value}` objects (not a dict); MCP server names should use underscores, not dashes, so LLM function-name namespacing stays clean.
