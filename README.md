...
- [ ] **Provenance schema depth (citation polarity + storage/influence policy split).** Sourced from @hegu-1's [openclaw#7707 reply](https://github.com/openclaw/openclaw/issues/7707) (2026-07-06). Two genuinely novel additions worth adopting: (a) `citation_polarity` (`supporting` / `refuting` / `neutral`) on retrieved facts — closes a real gap where access-count-driven trust can invert if a poisoned memory is retrieved frequently during refutation; (b) two-axis policy split (storage policy: is this claim allowed to be retained? / influence policy: is this claim allowed to affect planning, tool calls, or user-facing output?). Enterprise-deployment thinking — a claim can be stored as evidence but quarantined from decision-making until promoted. Reference architecture: <https://github.com/hegu-1/enterprise-ai-os-architecture>.
