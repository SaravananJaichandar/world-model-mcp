...
- [ ] **GitHub Copilot adapter (`install-copilot`).** Missed in the v0.10 breadth pass. Copilot has the largest addressable audience of any coding-AI runtime by 10-20x (millions of paid users vs Cursor's ~1M vs Claude Code's ~100K vs everything else). Copilot Chat in VS Code supports MCP server registration via `settings.json`; the adapter effort is roughly comparable to Continue. Last-mile verification needs a live VS Code session with Copilot Chat, same limitation as Continue.
- [ ] **Universal content-type routing.** v0.11.1 shipped the `content_type` schema field (`rule` / `fact` / `procedure`); v0.12 lands the consumers that route writes by content_type — not just in the Hermes `MemoryProvider`'s `handle_tool_call` dispatch, but across every supported runtime that has a built-in "notepad" memory layer (Cursor Memories, Codex memories, Continue memory, Hermes `MEMORY.md`). Injection semantics: `rule` always-inject at PostCompact; `fact` search-on-demand via `query_fact`; `procedure` never auto-inject. Reframes 5 of 7 adapters' value proposition beyond "additive tool surface" — the underlying axis (agent overuses one bucket for rules + facts + procedures) is universal across every runtime that ships an opinionated built-in memory store.
- [ ] **Storage vs influence policy split + hard `expires_at`.** Two additive schema changes that close enterprise-memory-governance gaps our current schema doesn't handle. (a) Nullable `influence_state` field (`observed` / `pending_review` / `approved` / `blocked`) distinct from the existing `status` and `severity` axes; consumers that retrieve facts for planning check `influence_state` while consumers that retrieve for audit see everything. Enables the pattern "log messy inputs as observations, quarantine them from decision-making until promoted or confirmed." (b) Nullable `expires_at` timestamp distinct from `last_decay_at`; continuous decay handles gradual confidence erosion, hard expiry handles the failure modes decay doesn't (compliance-driven retention, session tokens, credentials, ephemeral state). Together ~1-2 sessions, all nullable, all backward-compatible.
...
- [ ] **Citation polarity on retrieved facts** (`supporting` / `refuting` / `neutral`). Closes a real failure mode where access-count-driven trust can invert if a poisoned memory is retrieved frequently during refutation. Deferred from near-term because populating the field requires the retrieval caller to know intent, which we don't control at the schema layer alone — would ship a mostly-NULL field without adapter-side cooperation. Revisit when a specific integrator commits to instrumenting the annotation.
